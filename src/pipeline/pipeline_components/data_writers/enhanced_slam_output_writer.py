...
class EnhancedSLAMOutputWriter(AbstractDataWriter):
    ...
    def _run(self, step_nr: int, 
             ...
             closest_point_3d=None, distance_3d=None,
             closest_point_floor=None, distance_floor=None,
        ...
        Args:
            ...
            closest_point_3d: Current closest 3D point
            distance_3d: Current 3D distance to closest point
        ...
        if self.save_closest_points:
            if closest_point_3d is not None and distance_3d is not None:
                self.accumulated_closest_points_3d.append(
                    closest_point_3d.tolist() if isinstance(closest_point_3d, np.ndarray) else closest_point_3d
                )
                self.accumulated_distances_3d.append(float(distance_3d))
    ...
    def _save_closest_points_data(self) -> pathlib.Path:
        ...
        if self.analysis_format == 'json':
            data = {
                ...
                "analysis_summary": {
                    "avg_distance_3d": float(np.mean(self.accumulated_distances_3d)) if self.accumulated_distances_3d else None,
                    "min_distance_3d": float(np.min(self.accumulated_distances_3d)) if self.accumulated_distances_3d else None,
                    "max_distance_3d": float(np.max(self.accumulated_distances_3d)) if self.accumulated_distances_3d else None,
            ...
            with open(closest_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['step', 'closest_3d_x', 'closest_3d_y', 'closest_3d_z', 'distance_3d',
    ...
    def _save_metadata(self, results: Dict[str, Any], step_nr: int) -> None:
        ...
        if len(self.accumulated_distances_3d) > 0:
            metadata["distance_analysis"] = {
                "avg_distance_3d": float(np.mean(self.accumulated_distances_3d)),
                "min_distance_3d": float(np.min(self.accumulated_distances_3d)),
                "max_distance_3d": float(np.max(self.accumulated_distances_3d)),
                "std_distance_3d": float(np.std(self.accumulated_distances_3d))
            }
