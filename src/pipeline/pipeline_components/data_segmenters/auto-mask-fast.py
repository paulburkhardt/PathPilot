...
from build_sam import build_sam2_video_predictor, build_sam2
...
def sample_points_from_mask(mask):
    ...
    # Randomly sample one point from True values
...
def search_new_obj(masks_from_prev, mask_list, mask_ratio_thresh=0, ratio=0.5, area_threash=5000):
    ...
    # Calculate mask_none, representing areas not included in any previous mask
    ...
    for mask in mask_list:
        ...
        # Break early if we've covered enough of the uncovered area
        if mask_none.sum() / (mask_none.shape[0] * mask_none.shape[1]) <= mask_ratio_thresh:
            break
            
    print(f"Final no-mask ratio: {mask_none.sum() / (mask_none.shape[0] * mask_none.shape[1])}")
...
def extract_frames_from_video(video_path, output_dir, ext="jpg"):
    """
    Extracts frames from an mp4 video and saves them as numbered JPEGs in output_dir.
    """
    ...
    while success:
        out_path = os.path.join(output_dir, f"{frame_idx:06}.{ext}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--extract_frames", action="store_true", help="If set, extract frames from the video and exit.")
    parser.add_argument("--frames_dir", type=str, default=None, help="Directory to save extracted frames (if --extract_frames is set). Defaults to output_dir/frames.")
    ...
    if args.extract_frames:
        ...
        extract_frames_from_video(args.video_path, frames_dir)
        exit(0)
    ...
    sam2_checkpoint = os.path.join(project_root, 'segment-anything-2/checkpoints/sam2.1_hiera_tiny.pt')
    ...
    for now_frame in range(0, len(frame_names), args.detect_stride):
        ...
        print(f"is_key_frame: {is_key_frame}")
        ...
        if now_frame == 0: # first frame
            ...
            save_masks([masks[ann_obj_id]['segmentation'] for ann_obj_id in ann_obj_id_list], now_frame, os.path.join(base_dir, 'mask_each_frame-sam2'))
            ...
            if os.getenv("ONLY_STATISTIC","f") == 't':
                ...
                img.save(os.path.join(base_dir, 'mask_each_frame-sam2', 'all.png'))
                print(f"num:{len(ann_obj_id_list)}")
                print(f"no mask ratio:{all_mask.sum()/(width*height)}")
                exit()
            ...
            save_masks([mask['segmentation'] for mask in masks], now_frame, os.path.join(base_dir, 'mask_each_frame-sam2'))
            for id,mask in enumerate(masks_from_prev):
                if mask.sum() == 0:
                    continue
                prompts_loader.add(id, now_frame, mask[0])
            if is_key_frame:
                new_mask_list = search_new_obj(masks_from_prev, masks, mask_ratio_thresh=mask_ratio_thresh)
        ...
        plt.close("all")
        save_dir = os.path.join(base_dir,f"mask_each_frame_sam2")
        os.makedirs(save_dir,exist_ok=True)
        ...
        is_key_frame = no_mask_ratio > mask_ratio_thresh + 0.01
        print(f"mask_ratio_thresh: {mask_ratio_thresh}")
        masks_from_prev = out_mask_list


    # ###### Final output ######
    # save_dir = os.path.join(base_dir,"final-output")
...
# python auto-mask-fast.py --video_path ../../../../data/videos/chair_trash_walking --extract_frames store_true  --frames_dir ../../../../outputs/chair_trash_walking --output_dir ../../../../outputs/one_chair/segmentation_1
# python auto-mask-fast.py --video_path ../../../../outputs/chair_trash_walking --output_dir ../../../../outputs/chair_trash_walking/segmentation_1